...
### ۱.۷ پارس هوشمند پیامک و پیام‌های بانکی (`parseBankMessage`)

- تابع `parseBankMessage(t, base)` و هوک به ابتدای `parseLifeText` در `server.js`، `cloudflare/worker.js` و `cloudflare/header.js`.
- پشتیبانی کامل از کپی پیام اپ‌های بانکی (شامل فیلدهای «مبلغ: / بابت: / تاریخ:» و نادیده گرفتن سطر «موجودی:»).
- پشتیبانی از پیامک‌های بانکی واریز و برداشت با مبالغ ریال (تبدیل خودکار به تومان با تقسیم بر ۱۰).
- استخراج هوشمند تاریخ شمسی (`jalaliToGregorianIso`) و تاریخ انگلیسی (`Sep 14, 2026`).
- تعیین خودکار نوع تراکنش (`income`/`expense`) بر اساس کلیدواژه‌های واریز/دریافت/برداشت.
- افزودن ۶ تست کامل در `test/smoke.js` (بخش ۴۰) و پاس شدن همه تست‌ها.
