...
const specialCases = [
  ["let buf=Buffer.from(m[3],'base64');if(buf.length>12*1024*1024)return json(res,400,{error:'حجم فایل حداکثر ۱۲ مگابایت.'});let filename=id()+'.'+ext;fs.writeFileSync(path.join(UPLOADS_DIR,filename),buf);r.fileUrl='/uploads/'+filename;r.fileMime=mime;r.fileName=d.fileName||filename;r.fileSize=buf.length;write(db);return json(res,200,r)}", "let buf=bytesFromBase64(m[3]);if(buf.byteLength>12*1024*1024)return json(res,400,{error:'حجم فایل حداکثر ۱۲ مگابایت.'});let filename=id()+'.'+ext;if(!TELEGRAM_BOT_TOKEN||!user.telegramUserId)return json(res,503,{error:'برای آپلود فایل، اول بات تلگرام را از تنظیمات → اتصال‌ها وصل کن.'});let tgFileId=await tgSendDocument(user.telegramUserId,buf,d.fileName||filename,mime,'📎 سند: '+(r.title||''));r.fileUrl='/uploads/'+filename;r.fileTgId=tgFileId;r.fileMime=mime;r.fileName=d.fileName||filename;r.fileSize=buf.byteLength;await write(db);return json(res,200,r)}"],
