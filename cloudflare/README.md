...
این پوشه اپ «هِسته» رو به‌صورت Cloudflare Worker آماده می‌کنه — به‌جای فایل `server.js` که روی سیستم خودت اجرا می‌شه، این نسخه همیشه‌روشنه و آدرس ثابت داره.

## چیزهایی که همین الان واقعی و آماده‌ست

- **دیتابیس D1** به نام `pdmaz-db` ساخته شده و داده‌های فعلی‌ات (حساب کاربری، روزنگارها) داخلش کپی شده.
- **کد کامل Worker** در [`worker.js`](worker.js) — پورت کامل و تست‌شدهٔ همهٔ ۹۵ مسیر API از `server.js`.
- **`wrangler.toml`** با تنظیمات دیتابیس، فایل‌های استاتیک (`public/`)، و Cron هر ۱۵ دقیقه (برای گزارش تلگرام و قیمت‌های زنده، چون Worker برخلاف `server.js` نمی‌تونه یک تایمر دائمی داشته باشه).
