...
function makeHelpers(env) {
  ...
  function parseBankMessage(t,base){
    ...
    const mSms=t.match(/([\d.,،]+)\s*ریال\s*(?:از حساب شما\s*(?:پرید|کسر شد|برداشت شد|خارج شد|کم شد)|به حساب شما\s*(?:واریز شد|افزوده شد|اضافه شد|افزایش یافت))/);
    const mSms2=t.match(/(?:از حساب شما\s*(?:پرید|کسر شد|برداشت شد|خارج شد|کم شد)|به حساب شما\s*(?:واریز شد|افزوده شد|اضافه شد|افزایش یافت))[^\d]{0,30}?([\d.,،]+)\s*ریال/);
    if(!(mFor||mAmt||mSms||mSms2))return null;
    let amount=null,kind="expense";
    if(mAmt){amount=Number(String(mAmt[1]).replace(/[،,\s]/g,""));if(mAmt[2]==="ریال")amount=Math.round(amount/10)}
    if((amount==null||!isFinite(amount)||amount<=0)&&(mSms||mSms2)){const rial=Number(String((mSms||mSms2)[1]).replace(/[،,\s]/g,""));amount=Math.round(rial/10)}
